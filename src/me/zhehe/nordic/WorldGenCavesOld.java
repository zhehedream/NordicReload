...
import static me.zhehe.nordic.MathHelper.cos;
import static me.zhehe.nordic.MathHelper.floor;
import static me.zhehe.nordic.MathHelper.sin;
...
public class WorldGenCavesOld extends WorldGenBaseOld {
                ...
                for (int localZ = startZ; localZ < endZ; localZ++) {
                    ...
                    for (int localY = endY; localY >= startY; localY--) {
                        ...
                        if (yDistanceScaled > -0.7D &&
                            ...
                            if (previousBlock == Material.GRASS_BLOCK) {
                                hitGrassSurface = true;
                            }
                            if (previousBlock == Material.STONE
                                    || previousBlock == Material.DIRT
                                    || previousBlock == Material.GRASS_BLOCK) {
                                if (localY < 10) {
                                    data.setBlock(localX, localY + 1, localZ, Material.LAVA);
                                } else {
                                    data.setBlock(localX, localY + 1, localZ, Material.AIR);
                                    if (hitGrassSurface && data.getType(localX, localY, localZ) == Material.DIRT) {
                                        data.setBlock(localX, localY , localZ, Material.GRASS_BLOCK);
                                    }
                                }
                            }
                        }
        ...
        for (int xPos = startX; xPos < endX; xPos++) {
            for (int zPos = startZ; zPos < endZ; zPos++) {
                for (int yPos = endY + 1; yPos >= startY - 1; yPos--) {
                    ...
                    if (data.getType(xPos, yPos, zPos) == Material.WATER) {
                        return true;
                    }
    ...
    protected void generate(World world, int structureOriginChunkX, int structureOriginChunkZ, int generatedChunkX, int generatedChunkZ, ChunkGenerator.ChunkData data) {
        ...
        for (int i = 0; i < attempts; i++) {
            ...
            double structureOriginBlockY = rand.nextInt(rand.nextInt(36) + 8); //120->36
